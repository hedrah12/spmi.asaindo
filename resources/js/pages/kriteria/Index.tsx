...
import { Head, usePage, router } from "@inertiajs/react"; // Import router & usePage
...
export default function KriteriaIndex({ data, filters }) { // Terima props dari Controller
    ...
    // search state (menggunakan router inertia untuk search server-side)
    ...
    // Handle Search dengan Debounce (agar tidak request setiap ketik huruf)
    useEffect(() => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (isEditMode && selectedId) {
            // Update Data
            router.put(`/kriteria/${selectedId}`, formData, {
                onSuccess: () => closeModal(),
            });
        } else {
            // Create Data
            router.post("/kriteria", formData, {
                onSuccess: () => closeModal(),
            });
        }
    };
    ...
    // edit handler
    const handleEdit = (item: any) => { // item dari database
        setSelectedId(item.id);
        setIsEditMode(true);
        setIsModalOpen(true);

        setFormData({
            namaKriteria: item.kriteria, // Sesuaikan dengan nama kolom DB
        });
    };
                    ...
                    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                        ...
                        <DialogContent className="sm:max-w-[630px]">
                            <DialogHeader>
                                ...
                                <DialogDescription>Isi data berikut lalu klik simpan.</DialogDescription>
                            </DialogHeader>

                            <form onSubmit={handleSubmit} className="grid gap-6 py-4">
                                <div className="grid grid-cols-4 items-center gap-4">
                                    <Label htmlFor="namaKriteria" className="text-right">
                                        Nama Kriteria
                                    </Label>
                                    <Input
                                        id="namaKriteria"
                                        ...
                                        placeholder="Contoh: C.1. Visi Misi"
                                        required
                                    />
                                </div>
                            ...
                            {data.length === 0 ? (
                                ...
                                data.map((item, index) => (
                                    <tr key={item.id} className="border-t">
                                        ...
                                        {/* Sesuaikan nama properti dengan database 'kriteria' */}
